...
async def process_image(file: UploadFile = File(...)):
    ...
    logger.info(f"[process-image] 응답: order={order}, features_len={len(features_base64)}, coordinate={coordinate}")
...
async def process_image_crop(file: UploadFile = File(...)):
    logger.info(f"[process-image/crop] 요청 수신: {file.filename}")
    image_data = await file.read()
    image = Image.open(BytesIO(image_data)).convert("RGB")
    logger.info(f"[process-image/crop] 이미지 크기: {image.size}")
    ...
    logger.info(f"[process-image/crop] 응답: order={order}, features_len={len(features_base64)}")
    return JSONResponse(content={
        "order": order,
        "featuresBase64": features_base64
    })
