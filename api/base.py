...
def pq_encode(embedding: np.ndarray, codebook: np.ndarray) -> bytes:
    """
    Product Quantization 인코딩.

    Args:
        embedding: (D,) float32 — 원본 임베딩 (예: ResNet-50 의 2048-dim)
        codebook:  (M, K, D_sub) float32 — 코드북. D == M * D_sub 이어야 함.

    Returns:
        M byte 코드열. 각 byte 는 해당 sub-vector 에 가장 가까운 코드북 엔트리(0~K-1).
    """
    M, _, D_sub = codebook.shape
    codes = np.empty(M, dtype=np.uint8)
    for m in range(M):
        sub = embedding[m * D_sub:(m + 1) * D_sub]      # (D_sub,)
        # L2 거리 — sqrt 생략 (argmin 결과 동일)
        dists = np.sum((codebook[m] - sub) ** 2, axis=1)
